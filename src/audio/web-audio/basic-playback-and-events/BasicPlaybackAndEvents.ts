...
export default class BasicPlaybackAndEvents extends Phaser.Scene
{
	...
	create()
	{
		...
		const first = this.sound.add('overture', { loop: true }) as Phaser.Sound.WebAudioSound
		const second = this.sound.add('overture', { loop: true }) as Phaser.Sound.WebAudioSound
		const audioSprite = this.sound.addAudioSprite('creatures') as Phaser.Sound.WebAudioSound
